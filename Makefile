...
KIND_CLUSTER ?= mloperator-test-e2e
...
docker-build: ## Build docker image with the manager.
	$(CONTAINER_TOOL) build -t ${IMAGE_NAME}:${RELEASE_VERSION} .

.PHONY: docker-push
docker-push: ## Push docker image with the manager.
	$(CONTAINER_TOOL) push ${IMAGE_NAME}:${RELEASE_VERSION}
...
docker-multi-arch: ## Build and push docker multi-arch image with the manager.
	...
	$(CONTAINER_TOOL) buildx build --push -t ${IMAGE_NAME}:${RELEASE_VERSION} --platform linux/amd64,linux/arm64 .
...
pull-kf:
	...
	$(CONTROLLER_GEN) $(CRD_OPTIONS) paths=github.com/kubeflow/mpi-operator/pkg/apis/kubeflow/v1 output:crd:artifacts:config=config/crd/bases
