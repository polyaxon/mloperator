...
const (
	...
	// ZeroValue
	ZeroValue = 0
	// Min ttl in seconds to give the operator time to read final status before cleanup
	MinTTLValue = 5
	// DefaultNumReplicas
...
func GetTTL(ttlSecondsAfterFinished *int32) *int32 {
	...
	if ttlSecondsAfterFinished == nil || *ttlSecondsAfterFinished < MinTTLValue {
		// Default to 5 seconds to give the operator time to read final status before cleanup
		// Setting to 0 causes immediate deletion and race conditions with status updates
		defaultTtl := int32(MinTTLValue)
