...
func (r *KfJobReconciler) reconcileTFJobOp(ctx context.Context, instance *apiv1.KfJob) (ctrl.Result, error) {
	// Reconcile the underlaying job
	return ctrl.Result{}, r.reconcileTFJob(ctx, instance)
}

func (r *KfJobReconciler) reconcileTFJob(ctx context.Context, instance *apiv1.KfJob) error {
	log := r.Log
	...
	if err != nil && apierrs.IsNotFound(err) {
		...
		if shouldMarkDeleted {
			log.Info("TFJob was deleted externally, marking operation as stopped", "namespace", instance.Namespace, "name", instance.Name)
			if updated := instance.Status.MarkJobAsDeleted("TFJob", false); updated {
		...
		if err != nil {
			if updated := instance.Status.LogWarning("OperatorCreateTFJob", err.Error()); updated {
	...
	// Don't update if: 1) just created, 2) instance is done, 3) TFJob is in terminal state
...
func (r *KfJobReconciler) reconcileTFJobStatus(instance *apiv1.KfJob, job unstructured.Unstructured) (bool, error) {
	return r.reconcileKfJobStatus(instance, job)
}

func (r *KfJobReconciler) cleanUpTFJob(ctx context.Context, instance *apiv1.KfJob) (ctrl.Result, error) {
