...
func (r *ClusterReconciler) delete(ctx context.Context, instance *apiv1.Cluster) error {
	log := r.Log
...
func (r *ClusterReconciler) handleTTL(ctx context.Context, instance *apiv1.Cluster) (ctrl.Result, error) {
	log := r.Log
...
func (r *ClusterReconciler) handlePastActiveDeadline(ctx context.Context, instance *apiv1.Cluster) (*time.Duration, error) {
	log := r.Log
	...
	if duration >= allowedDuration {
		...
		if updated := instance.Status.LogStopped("DeadlineExceeded", "Cleanup triggered based on ActiveDeadlineSeconds, operation was active longer than specified deadline"); updated {
...
func (r *ClusterReconciler) getBackOff(backOff int32) time.Duration {
